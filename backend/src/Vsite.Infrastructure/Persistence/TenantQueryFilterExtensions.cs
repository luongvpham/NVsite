...
///
/// ⚠️ Filter tham chiếu `dbContext.TenantContext` (property trên CHÍNH instance AppDbContext, không
...
{
    public static void ApplyGlobalFilters(this ModelBuilder modelBuilder, AppDbContext dbContext)
    {
        var dbContextConstant = Expression.Constant(dbContext);
        var tenantContextAccess = Expression.Property(dbContextConstant, nameof(AppDbContext.TenantContext));
