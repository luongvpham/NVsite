...
/// khớp với `UserShop.PasswordHash`, không bao giờ rơi về `User.PasswordHash`).
/// </summary>
public sealed class LoginHandler(IIdentityDbContext db, ITenantContext tenantContext, IPasswordHasher<User> passwordHasher, IJwtTokenService tokenService)
    : IRequestHandler<LoginCommand, AuthTokenResult>
{
    ...
    public async Task<AuthTokenResult> Handle(LoginCommand request, CancellationToken cancellationToken)
    {
        ...
        if (tenantContext.AudienceKind == TenantAudienceKind.Shop)
        {
            ...
            // Chưa có UserShop → chưa từng đặt password riêng ở shop này, không có gì để so khớp
            // (03 §3.3 — phải đăng ký ở shop đó trước, không "login thẳng" bằng credential global).
        ...
        if (passwordHashToVerify is null ||
